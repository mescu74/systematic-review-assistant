...
@pytest.fixture
def search_result_repo() -> SearchResultRepository:
...
def test_create_review(mock_session: MagicMock) -> None:
...
def test_get_review(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_id = uuid.uuid4()
...
def test_update_review(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_id = uuid.uuid4()
...
def test_delete_review(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_id = uuid.uuid4()
...
def test_get_all_reviews(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    expected = [
        SystematicReview(
            id=uuid.uuid4(),
...
def test_list_reviews(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    expected = [
        SystematicReview(
            id=uuid.uuid4(),
...
def test_get_review_with_search_results(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_id = uuid.uuid4()
    expected = SystematicReview(
        id=review_id,
...
def test_get_review_with_all(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_id = uuid.uuid4()
    expected = SystematicReview(
        id=review_id,
...
def test_base_repo_add_constraint_error(mock_session: MagicMock) -> None:
    repo = SystematicReviewRepository()
    review_to_add = SystematicReview(
        id=uuid.uuid4(),
        research_question="test q",
    ...
    mock_session.flush.side_effect = IntegrityError(
        "duplicate key", params=None, orig=Exception("original db error")
    )
...
def test_search_result_repo_get_by_doi_found(
    ...
    result = search_result_repo.get_by_doi(mock_session, doi=sample_search_result.doi)
...
def test_search_result_repo_get_by_title_and_year_found(
    ...
    result = search_result_repo.get_by_title_and_year(
...
def test_search_result_repo_advanced_search_no_filters(
    ...
    results = search_result_repo.advanced_search(
...
def test_search_result_repo_advanced_search_pagination(
    ...
    search_result_repo.advanced_search(
...
def test_search_result_repo_advanced_search_error_handling(
    mock_session: MagicMock, search_result_repo: SearchResultRepository
) -> None:
    ...
    with pytest.raises(RepositoryError, match="DB search error"):
        search_result_repo.advanced_search(
            mock_session, search_params=SearchResultFilter()
        )
...
def test_search_result_repo_count_no_filters(
    ...
    count = search_result_repo.count(mock_session, search_params=None)
...
def test_search_result_repo_count_with_filters(
    ...
    count = search_result_repo.count(mock_session, search_params=search_filters)
...
def test_search_result_repo_count_error_handling(
    ...
    with pytest.raises(RepositoryError, match="DB count error"):
        search_result_repo.count(mock_session, search_params=SearchResultFilter())
