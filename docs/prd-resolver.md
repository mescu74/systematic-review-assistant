...
- The system must invoke the resolver LLM chain (using the defined `RESOLVER_MODEL` and `resolver_prompt` with structured output (`ResolverOutput`)) for each item in the disagreement batch.
...
- The system must parse the structured output (`ResolverOutput` schema) from the resolver LLM for each resolved item.
- The `ResolverOutput` schema currently includes `include: list[t.Literal["conservative", "comprehensive"]]`, `reasoning`, `confidence_score`, `review_id`, `search_result_id`.
- **Update:** The `ResolverOutput` schema and the corresponding prompt need refinement. The schema **must** be updated to include a single, final decision field: `final_decision: ScreeningDecisionType`. The prompt must instruct the LLM to provide this single decision (`INCLUDE`, `EXCLUDE`, or potentially `UNCERTAIN` if resolution isn't clear). The `include` list should be removed or repurposed.
