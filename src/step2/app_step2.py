...
from pubmed_integration import pubmed_search, pubmed_fetch_details
...
def run_step2_search_and_index():
    ...
    if "research_criteria" not in st.session_state:
        st.warning("Please complete Step 1 first to define your research question/criteria.")
        return
    ...
    if st.button("Run PubMed Search"):
        ...
        # Build LlamaIndex
        index = build_llama_index_from_pubmed(pubmed_records, index_path="pubmed_index.json")
        if index:
