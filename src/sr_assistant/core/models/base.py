...
Examples:
    >>> ret = supabase.table("reviews").insert(json.loads(review.model_dump_json(exclude=["created_at", "updated_at"]))).execute()
...
class PubMedResult(IdMixin, CreatedAtMixin, table=True):
    ...
    review_id: uuid.UUID = Field(title="Review ID", foreign_key="reviews.id", index=True)
    query: str = Field(description="PubMed search query", title="Query", schema_extra={"example": "(cancer) AND (immunotherapy) AND (clinical trial)"})

    # Article info
    pmid: str = Field(index=True, title="PMID", schema_extra={"example": "39844819"})
    pmc: str | None = Field(default=None, title="PMC ID", schema_extra={"example": "PMC11753851"})
    doi: str | None = Field(default=None, title="DOI", schema_extra={"example": "10.1155/jimr/5845167"})
