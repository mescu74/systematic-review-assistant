...
- AC3: `SearchResult.source_metadata` correctly stores `{"benchmark_human_decision": <parsed_human_decision>}` for each record, where `<parsed_human_decision>` is a boolean or string representation of the ground truth from the CSV.
