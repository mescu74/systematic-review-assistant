...
- [ ] **Task 2.1.1: SQLModel Changes (`models.py`) & Alembic Migration (User Coordinated and Step-by-Step):**
  - [ ] Add `final_decision: ScreeningDecisionType | None` field to the `SearchResult` SQLModel, ensuring correct DB type (enum), nullability, and indexing. Configure `sa_column` as needed.
  - [ ] Verify/Update the `ScreeningResolution` SQLModel according to `docs/prd-resolver.md#FR6-Store-Resolution-Data` and `docs/data-models.md`. Ensure all FKs (`search_result_id`, `review_id`, `conservative_result_id`, `comprehensive_result_id`) and other fields are correctly defined.
  - [ ] Verify other SQLModels (`SystematicReview`, `ScreenAbstractResult`) for alignment with `docs/data-models.md` and project standards (e.g., `SearchResult.year` as `str | None`, FKs, timestamps, JSONB types). Correct any minor misalignments.
  - [ ] **Propose Alembic Migration Script Generation (User Coordinated):**
    - [ ] After all SQLModel changes in `src/sr_assistant/core/models.py` are complete and confirmed, inform the user.
    - [ ] Propose the command for generating the migration script: `ENVIRONMENT=prototype uv run alembic revision --autogenerate -m "descriptive_migration_name"` (suggest a specific descriptive name).
    - [ ] Once the script is generated in `alembic/versions/`, present its full content to the user for **MANDATORY REVIEW AND EXPLICIT APPROVAL.**
  - [ ] **Propose Alembic Migration Application (Upgrade - User Coordinated):**
    - [ ] Only after obtaining explicit user approval for the generated script, propose the command for applying the migration: `ENVIRONMENT=prototype uv run alembic upgrade head`.
    - [ ] Await user confirmation of command execution and successful database schema verification by the user.
  - [ ] **Propose Alembic Migration Downgrade/Upgrade Test (User Coordinated):**
    - [ ] After successful upgrade and verification, propose testing downgrade: `ENVIRONMENT=prototype uv run alembic downgrade -1`. Await user execution and verification.
    - [ ] Then, propose re-applying the upgrade: `ENVIRONMENT=prototype uv run alembic upgrade head`. Await user execution and verification.
- [ ] **Task 2.1.2: Pydantic Schema Implementation/Refactoring (`schemas.py`):**
  - [ ] **General Standards Application:** Ensure all schemas inherit `BaseSchema` (where appropriate), use field docstrings, `collections.abc.Mapping`, `AwareDatetime` from `pydantic`. Remove `Field(description=...)` and `Field(ge/le=...)` for LLM output schemas (documenting constraints in docstrings instead).
  - [ ] **`SystematicReview` Schemas:**
    - [ ] Verify/Align `SystematicReviewCreate`.
    - [ ] Refactor `SystematicReviewUpdate` (all fields optional, resolve linter errors).
    - [ ] Verify/Align `SystematicReviewRead`.
  - [ ] **Screening Schemas (LLM I/O & Service):**
    - [ ] Verify/Align `ScreeningResponse` (LLM output - check confidence score docstring/field).
    - [ ] Verify/Align `ScreeningResult` (hydrated LLM output).
    - [ ] Create `ScreeningResultCreate` (service input).
    - [ ] Create `ScreeningResultUpdate` (service input).
    - [ ] Create `ScreeningResultRead` (service output).
  - [ ] **`ScreeningResolution` Schemas:**
    - [ ] Verify/Align `ScreeningResolutionSchema` (LLM output - check confidence score, clarify caller-populated fields in docstrings).
    - [ ] Create `ScreeningResolutionCreate` (service input).
    - [ ] Create `ScreeningResolutionRead` (service output).
  - [ ] **Suggestion Agent Schemas:**
    - [ ] Verify/Align `PicosSuggestions` (LLM output).
    - [ ] Verify/Align `SuggestionResult` (`TypedDict`).
- [ ] **Task 2.1.3: Repository Verification/Updates (`repositories.py`):**
  - [ ] Verify/Update/Create `ScreeningResolutionRepository` (inheriting `BaseRepository[ScreeningResolution]`) with methods: `add`, `get_by_id`, `get_by_search_result_id`. Write/update unit tests.
  - [ ] Ensure `SearchResultRepository` can efficiently update `final_decision` and `resolution_id` on `SearchResult` records (e.g., via its existing `update` method or a more specific one if needed). Write/update relevant unit tests.
- [ ] **Task 2.1.4: Linting & Final Checks:**
  - [ ] Perform a final linter pass (`ruff check --fix`, `ruff format`) on `schemas.py`, `models.py`, `repositories.py` to resolve any outstanding issues related to this story's changes.
  - [ ] Write/update unit tests for all new/modified Pydantic schemas in `tests/unit/core/test_schemas.py`.
